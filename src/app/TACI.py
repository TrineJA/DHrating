...
from data import df_circle as df
...
@app.callback(
    Output('t_ratings_reactive_taci', 'style_data_conditional'),
    [Input('t_ratings_reactive_taci', 'selected_rows')]
)
...
@app.callback(
    Output('f_ratings_reactive_container_taci', "children"),
    [Input('t_ratings_reactive_taci', 'selected_rows')])
def update_graphs(selected_rows):
    ...
    return [
        dcc.Graph(id='f_ratings_reactive_taci',
                  figure={
                      'data': [
                          go.Scatter(
