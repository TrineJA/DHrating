...
from data import df_wl as df

# define layout
layout = html.Div([
    html.Div(id='f_ratings_reactive_container_taud', style={'marginBottom': 30}),

    html.Div([
        dash_table.DataTable(id='t_ratings_reactive_taud',
                             columns=
...
@app.callback(
    Output('f_ratings_reactive_container_taud', "children"),
    [Input('t_ratings_reactive_taud', 'selected_rows')])
def update_graphs(selected_rows):
    ...
    return [
        dcc.Graph(id='f_ratings_reactive_taud',
                  figure={
                      'data': [
                          go.Scatter(
